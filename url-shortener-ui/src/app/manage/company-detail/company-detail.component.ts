...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, HttpClientModule], // FormsModule eklendi
...
export class CompanyDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      switchMap(params => {
        ...
        return this.companyService.getCompanyById(+id).pipe(
          catchError(error => {
            ...
            this.errorMessage = 'Firma detayları alınırken bir hata oluştu.';
            return of(null);
          })
        );
      })
    ...
    this.company$.subscribe(company => {
      ...
      // URL'lere isForever ata
      if (company && company.urls) {
        company.urls.forEach(url => {
          url.isForever = (url.expiresAt === null || url.expiresAt === '' || url.expiresAt === undefined);
        });
      }
  ...
  updateTokenUses(token: Token): void {
    if (!this.isSuperAdmin) {
      this.updateTokenError = 'Sadece süper admin token kullanım hakkını güncelleyebilir!';
      return;
    }
  ...
  shortenUrlForCompany(): void {
    ...
    if (!this.newLongUrl || !this.selectedTokenValue || (!this.newExpiresAt && !this.isForever)) {
      this.shorteningError = 'Lütfen kısaltılacak URL\'yi, son kullanma tarihini (veya süresiz seçeneğini) ve kullanılacak token\'i seçin.';
      console.warn('[URL KISALTMA][UYARI] URL, son kullanma tarihi veya token seçilmedi.');
      return;
    }
    const expiresAtToSend = this.isForever ? null : this.newExpiresAt;
    console.log(`[URL KISALTMA] Kullanıcı yeni URL kısaltıyor: ${this.newLongUrl}, Token: ${this.selectedTokenValue}, Son Kullanma: ${expiresAtToSend}`);
    this.urlService.shortenUrl(this.newLongUrl, this.selectedTokenValue, expiresAtToSend).subscribe({
      next: (response: any) => {
  ...
  deleteToken(tokenId: number) {
    if (!this.isSuperAdmin) {
      this.updateTokenError = 'Sadece süper admin token silebilir!';
      return;
    }
  ...
  updateUrlExpiresAt(url: Url) {
    const expiresAt: string | null = url.isForever ? null : (url.expiresAt ? String(url.expiresAt) : null);
    this.urlService.updateUrlExpiresAt(url.id, { expiresAt }).subscribe({
      next: () => {
        this.refreshCompanyDetails();
      },
      error: (err: any) => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          let markerType = accuracy <= 50 ? 'gps' : 'ip';
          this.konumUyari = accuracy <= 50 ? 
            `Hassas konum alındı (${Math.round(accuracy)} metre)` : 
            `Düşük hassasiyetli konum (${Math.round(accuracy)} metre)`;
          this.redirectWithLocation(url, lat, lng, markerType);
        },
        (error) => {
          // GPS ile konum alınamazsa IP bazlı konum al
          this.getIpBasedLocation(url, true);
        },
        options
      );
    } else {
      // Tarayıcı GPS desteklemiyorsa IP bazlı konum al
      this.getIpBasedLocation(url, true);
    }
  }

  private getIpBasedLocation(url: Url, showAlertIfFail: boolean = false) {
    fetch('https://ipapi.co/json/')
      .then(response => {
        if (!response.ok) {
          throw new Error('IP konum servisi şu anda kullanılamıyor.');
        }
        return response.json();
      })
      .then(data => {
        if (data.latitude && data.longitude) {
          this.konumUyari = 'IP bazlı yaklaşık konum alındı.';
          this.redirectWithLocation(url, data.latitude, data.longitude, 'ip');
        } else {
          if (showAlertIfFail) {
            alert('Konum alınamadı. Lütfen konum izni verin veya gizli sekme kullanmayın. Linke yönlendirme yapılmayacak.');
          }
        }
      })
      .catch(error => {
